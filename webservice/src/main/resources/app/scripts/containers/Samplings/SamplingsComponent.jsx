...
export default class ComponentSamplings extends Component{
  ...
  fetchData(){
    ...
    ViewModeREST.getSamplingEvents(topologyId,tempParams).then((results) => {
      if(results.responseMessage !== undefined){
        ...
        if(componentId !== '' && selectedComponentArr.length == 0){
          selectedComponents.push(this.getSelectedViewModeComponent(componentId,componentOptions));
        } else {
  ...
  getTableHeader = () => {
    ...
    const columnSelectButton = <DropdownButton
                                  ...
                                  <MenuItem eventKey="keyValues">
                                    {showColumn.keyValues ?
                                      <i className="fa fa-check-square-o" aria-hidden="true"></i>
                                      : <i className="fa fa-square-o" aria-hidden="true"></i>} key Values
                                    </MenuItem>
                                </DropdownButton>;
                ...
                <FormGroup className="col-sm-6">
                  <label>Date / Time :</label>
                    <div className="input-group add-on">
                      <input defaultValue={start+' - '+end} ref="datePicker" className="form-control" onFocus={this.toggleDatePicker} name="datePicker" type="text" />

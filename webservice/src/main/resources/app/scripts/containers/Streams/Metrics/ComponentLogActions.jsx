...
class ComponentLogActions extends Component {
  ...
  handleInputChange = (e) => {
    ...
    this.showError = Number(e.target.value) < 1 || Number(e.target.value) > 101 ? true : false;
    if(refType !== undefined){
      this.inputFlag = true;
  ...
  handleKeyPress = (nodeId,e) => {
    ...
    if(e.keyCode === 13 && !this.showError && refType === undefined){
      this.handleCallBackAction('SAMPLE',nodeId,Number(e.target.value));
      this.inputFlag = true;
    }
  }
  ...
  render() {
    ...
    if(allComponentLevelAction){
      ...
      sampleVal = refType === undefined && samplingObj !== undefined && samplingObj.enabled ?  samplingObj.duration : samlpingValue !== undefined ? samlpingValue : '';
    }

...
class TopologyViewMode extends Component {
  ...
  toggleLogLevelDropdown = (isOpen) => {
    ...
    if(!isOpen && logTopologyLevel !== 'None' && durationTopologyLevel > 0 && sampleTopologyLevel > 0) {
      this.props.topologyLevelDetailsFunc('SAMPLE','enable');
      this.postTopologyLogConfigCallBack(logTopologyLevel,durationTopologyLevel);
    } else if(!isOpen && sampleTopologyLevel > 0 && logTopologyLevel === 'None' && durationTopologyLevel <= 0){
      this.props.topologyLevelDetailsFunc('SAMPLE','enable');

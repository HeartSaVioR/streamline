...
class TopologyViewMode extends Component {
  ...
  toggleLogLevelDropdown = (isOpen) => {
    let {logTopologyLevel, durationTopologyLevel} = this.props.viewModeData;
  ...
  render() {
    ...
    const titleContent = (
      <span>All Components &emsp; {displayTime == '0:0' ? '' : (<span>Timer: <span style={{color: '#d06831'}}>{displayTime}</span> &emsp; </span>)}Log: <span style={{color: '#2787ad'}}>{_.capitalize(this.state.logLevel)}</span> &emsp; Sampling: <span style={{color: '#2787ad'}}>{viewModeData.sampleTopologyLevel}%</span></span>
    );
